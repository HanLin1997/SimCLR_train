...
class MLP(nn.Module):
    def __init__(self, num_in, num_hid):
        super(MLP, self).__init__()
        self.num_in = num_in
        self.avgpool = nn.AdaptiveAvgPool2d((1, 1))
        self.fc1 = nn.Linear(num_in, num_hid)
        self.batch_norm = nn.BatchNorm1d(num_hid)
        self.fc2 = nn.Linear(num_hid, 260)

    def forward(self, x):
        B,C,H,W = x.shape
        x = self.avgpool(x)
        x = x.reshape(B, self.num_in)
        x = F.relu(self.fc1(x))
        x = self.batch_norm(x)
        x = self.fc2(x)
